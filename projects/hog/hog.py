...
def roll_dice(num_rolls, dice=six_sided):
    ...
    sum = 0
    i = 1
    one = False #indicate if there is one rolled out
    while i <= num_rolls:
        number = dice()
        if number > 1:
            sum = sum + number
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say1(score0, score1):
        ...
        if who == 1:
            score = score1
        difference = score - previous_score
        if difference == 1 and difference > previous_high:
            print ("1 point! That's the biggest gain yet for", "Player", who)
        elif difference > 1 and difference > previous_high:
            print (difference, "points! That's the biggest gain yet for", "Player", who)
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 10
    num = abs(opponent_score//10 - opponent_score%10)
    if num + 2 >= margin:
        return 0
    else:
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    """The swap_strategy rolls 0 if it would cause a beneficial swap.
    It also returns 0 if rolling 0 would give at least margin points
    and would not cause a swap. Otherwise, the strategy rolls num_rolls.
    """
    # BEGIN PROBLEM 11
    num = abs(opponent_score//10 - opponent_score%10) + 2
    new_score = score + num
    if new_score < opponent_score and opponent_score%new_score == 0:
        return 0
    if num >= margin and opponent_score%new_score != 0:
        return 0
    else:
        return num_rolls  # Replace this statement
    # END PROBLEM 11
